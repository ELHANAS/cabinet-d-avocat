...
@section('content')
    <div class="p-5" style="height:100%;background: {{\App\Models\Color::getColors()->bgMain}};color: {{\App\Models\Color::getColors()->textMain}}">
        <form class="row" action="{{route('tache.update')}}" method="post">
            @csrf
            <input hidden value="{{$tache->id}}" name="id">
            <div class="col-6 p-5">
                <div class="form-group row">
                    <label class="col" for="titre">Titre :</label>
                    <input  type="text" class="form-control  col col" id="titre" value="{{old("titre" , $tache->titre)}}" aria-describedby="emailHelp" name="titre" placeholder="titre">
                    @error("titre")
                ...
                <div class="form- row">
                    <label  class="form-label col" for="DTache">date de tâche :</label>
                    <input type="date" class="form-control col" value="{{old("date" , $tache->DTache)}}" id="DTache" name="date">
                    @error("date")
                    <small  class="ms-3 form-text text-danger">{{$message}}</small>
                    @enderror
                </div>
                <div class="form-group row">
                    ...
                    <select class="form-select col" name="affaire">
                        @foreach( $dataaffaire as $data)
                            <option @if($data->id == $tache->id_affaire || old("affaire") == $data->id) selected @endif value="{{$data->id}}">{{$data->name}}</option>
                        @endforeach
